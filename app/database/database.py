...
def create_tables_if_not_exist(logger=None):
    ...
    # Tabella: ml_signals
    # Stores every ML signal generated by the daily pipeline for backtesting.
    # One row per (as_of_date, collection_identifier, chain) run.
    cursor.execute("""
    ...
    """)
    if logger:
        logger.info("Tabella ml_signals creata.")

    cursor.execute("""
    CREATE INDEX IF NOT EXISTS idx_ml_signals_date ON ml_signals (as_of_date DESC);
    """)
    if logger:
        logger.info("Indice idx_ml_signals_date creato.")
